/* eslint-disable */
export const countries = [
  { code: 'US', label: 'United States', suggested: true },
  { code: 'AU', label: 'Australia', suggested: true },
  { code: 'CA', label: 'Canada', suggested: true },
  { code: 'DE', label: 'Germany', suggested: true },
  { code: 'FR', label: 'France', suggested: true },
  { code: 'JP', label: 'Japan', suggested: true },
  { code: 'AD', label: 'Andorra' },
  { code: 'AE', label: 'United Arab Emirates' },
  { code: 'AF', label: 'Afghanistan' },
  { code: 'AG', label: 'Antigua and Barbuda' },
  { code: 'AI', label: 'Anguilla' },
  { code: 'AL', label: 'Albania' },
  { code: 'AM', label: 'Armenia' },
  { code: 'AO', label: 'Angola' },
  { code: 'AQ', label: 'Antarctica' },
  { code: 'AR', label: 'Argentina' },
  { code: 'AS', label: 'American Samoa' },
  { code: 'AT', label: 'Austria' },
  { code: 'AW', label: 'Aruba' },
  { code: 'AX', label: 'Alland Islands' },
  { code: 'AZ', label: 'Azerbaijan' },
  { code: 'BA', label: 'Bosnia and Herzegovina' },
  { code: 'BB', label: 'Barbados' },
  { code: 'BD', label: 'Bangladesh' },
  { code: 'BE', label: 'Belgium' },
  { code: 'BF', label: 'Burkina Faso' },
  { code: 'BG', label: 'Bulgaria' },
  { code: 'BH', label: 'Bahrain' },
  { code: 'BI', label: 'Burundi' },
  { code: 'BJ', label: 'Benin' },
  { code: 'BL', label: 'Saint Barthelemy' },
  { code: 'BM', label: 'Bermuda' },
  { code: 'BN', label: 'Brunei Darussalam' },
  { code: 'BO', label: 'Bolivia' },
  { code: 'BR', label: 'Brazil' },
  { code: 'BS', label: 'Bahamas' },
  { code: 'BT', label: 'Bhutan' },
  { code: 'BV', label: 'Bouvet Island' },
  { code: 'BW', label: 'Botswana' },
  { code: 'BY', label: 'Belarus' },
  { code: 'BZ', label: 'Belize' },
  { code: 'CC', label: 'Cocos (Keeling) Islands' },
  { code: 'CD', label: 'Congo, Democratic Republic of the' },
  { code: 'CF', label: 'Central African Republic' },
  { code: 'CG', label: 'Congo, Republic of the' },
  { code: 'CH', label: 'Switzerland' },
  { code: 'CI', label: "Cote d'Ivoire" },
  { code: 'CK', label: 'Cook Islands' },
  { code: 'CL', label: 'Chile' },
  { code: 'CM', label: 'Cameroon' },
  { code: 'CN', label: 'China' },
  { code: 'CO', label: 'Colombia' },
  { code: 'CR', label: 'Costa Rica' },
  { code: 'CU', label: 'Cuba' },
  { code: 'CV', label: 'Cape Verde' },
  { code: 'CW', label: 'Curacao' },
  { code: 'CX', label: 'Christmas Island' },
  { code: 'CY', label: 'Cyprus' },
  { code: 'CZ', label: 'Czech Republic' },
  { code: 'DJ', label: 'Djibouti' },
  { code: 'DK', label: 'Denmark' },
  { code: 'DM', label: 'Dominica' },
  { code: 'DO', label: 'Dominican Republic' },
  { code: 'DZ', label: 'Algeria' },
  { code: 'EC', label: 'Ecuador' },
  { code: 'EE', label: 'Estonia' },
  { code: 'EG', label: 'Egypt' },
  { code: 'EH', label: 'Western Sahara' },
  { code: 'ER', label: 'Eritrea' },
  { code: 'ES', label: 'Spain' },
  { code: 'ET', label: 'Ethiopia' },
  { code: 'FI', label: 'Finland' },
  { code: 'FJ', label: 'Fiji' },
  { code: 'FK', label: 'Falkland Islands (Malvinas)' },
  { code: 'FM', label: 'Micronesia, Federated States of' },
  { code: 'FO', label: 'Faroe Islands' },
  { code: 'GA', label: 'Gabon' },
  { code: 'GB', label: 'United Kingdom' },
  { code: 'GD', label: 'Grenada' },
  { code: 'GE', label: 'Georgia' },
  { code: 'GF', label: 'French Guiana' },
  { code: 'GG', label: 'Guernsey' },
  { code: 'GH', label: 'Ghana' },
  { code: 'GI', label: 'Gibraltar' },
  { code: 'GL', label: 'Greenland' },
  { code: 'GM', label: 'Gambia' },
  { code: 'GN', label: 'Guinea' },
  { code: 'GP', label: 'Guadeloupe' },
  { code: 'GQ', label: 'Equatorial Guinea' },
  { code: 'GR', label: 'Greece' },
  {
    code: 'GS',
    label: 'South Georgia and the South Sandwich Islands',
  },
  { code: 'GT', label: 'Guatemala' },
  { code: 'GU', label: 'Guam' },
  { code: 'GW', label: 'Guinea-Bissau' },
  { code: 'GY', label: 'Guyana' },
  { code: 'HK', label: 'Hong Kong' },
  { code: 'HM', label: 'Heard Island and McDonald Islands' },
  { code: 'HN', label: 'Honduras' },
  { code: 'HR', label: 'Croatia' },
  { code: 'HT', label: 'Haiti' },
  { code: 'HU', label: 'Hungary' },
  { code: 'ID', label: 'Indonesia' },
  { code: 'IE', label: 'Ireland' },
  { code: 'IL', label: 'Israel' },
  { code: 'IM', label: 'Isle of Man' },
  { code: 'IN', label: 'India' },
  { code: 'IO', label: 'British Indian Ocean Territory' },
  { code: 'IQ', label: 'Iraq' },
  { code: 'IR', label: 'Iran, Islamic Republic of' },
  { code: 'IS', label: 'Iceland' },
  { code: 'IT', label: 'Italy' },
  { code: 'JE', label: 'Jersey' },
  { code: 'JM', label: 'Jamaica' },
  { code: 'JO', label: 'Jordan' },
  { code: 'KE', label: 'Kenya' },
  { code: 'KG', label: 'Kyrgyzstan' },
  { code: 'KH', label: 'Cambodia' },
  { code: 'KI', label: 'Kiribati' },
  { code: 'KM', label: 'Comoros' },
  { code: 'KN', label: 'Saint Kitts and Nevis' },
  { code: 'KP', label: "Korea, Democratic People's Republic of" },
  { code: 'KR', label: 'Korea, Republic of' },
  { code: 'KW', label: 'Kuwait' },
  { code: 'KY', label: 'Cayman Islands' },
  { code: 'KZ', label: 'Kazakhstan' },
  { code: 'LA', label: "Lao People's Democratic Republic" },
  { code: 'LB', label: 'Lebanon' },
  { code: 'LC', label: 'Saint Lucia' },
  { code: 'LI', label: 'Liechtenstein' },
  { code: 'LK', label: 'Sri Lanka' },
  { code: 'LR', label: 'Liberia' },
  { code: 'LS', label: 'Lesotho' },
  { code: 'LT', label: 'Lithuania' },
  { code: 'LU', label: 'Luxembourg' },
  { code: 'LV', label: 'Latvia' },
  { code: 'LY', label: 'Libya' },
  { code: 'MA', label: 'Morocco' },
  { code: 'MC', label: 'Monaco' },
  { code: 'MD', label: 'Moldova, Republic of' },
  { code: 'ME', label: 'Montenegro' },
  { code: 'MF', label: 'Saint Martin (French part)' },
  { code: 'MG', label: 'Madagascar' },
  { code: 'MH', label: 'Marshall Islands' },
  {
    code: 'MK',
    label: 'Macedonia, the Former Yugoslav Republic of',
  },
  { code: 'ML', label: 'Mali' },
  { code: 'MM', label: 'Myanmar' },
  { code: 'MN', label: 'Mongolia' },
  { code: 'MO', label: 'Macao' },
  { code: 'MP', label: 'Northern Mariana Islands' },
  { code: 'MQ', label: 'Martinique' },
  { code: 'MR', label: 'Mauritania' },
  { code: 'MS', label: 'Montserrat' },
  { code: 'MT', label: 'Malta' },
  { code: 'MU', label: 'Mauritius' },
  { code: 'MV', label: 'Maldives' },
  { code: 'MW', label: 'Malawi' },
  { code: 'MX', label: 'Mexico' },
  { code: 'MY', label: 'Malaysia' },
  { code: 'MZ', label: 'Mozambique' },
  { code: 'NA', label: 'Namibia' },
  { code: 'NC', label: 'New Caledonia' },
  { code: 'NE', label: 'Niger' },
  { code: 'NF', label: 'Norfolk Island' },
  { code: 'NG', label: 'Nigeria' },
  { code: 'NI', label: 'Nicaragua' },
  { code: 'NL', label: 'Netherlands' },
  { code: 'NO', label: 'Norway' },
  { code: 'NP', label: 'Nepal' },
  { code: 'NR', label: 'Nauru' },
  { code: 'NU', label: 'Niue' },
  { code: 'NZ', label: 'New Zealand' },
  { code: 'OM', label: 'Oman' },
  { code: 'PA', label: 'Panama' },
  { code: 'PE', label: 'Peru' },
  { code: 'PF', label: 'French Polynesia' },
  { code: 'PG', label: 'Papua New Guinea' },
  { code: 'PH', label: 'Philippines' },
  { code: 'PK', label: 'Pakistan' },
  { code: 'PL', label: 'Poland' },
  { code: 'PM', label: 'Saint Pierre and Miquelon' },
  { code: 'PN', label: 'Pitcairn' },
  { code: 'PR', label: 'Puerto Rico' },
  { code: 'PS', label: 'Palestinestate of' },
  { code: 'PT', label: 'Portugal' },
  { code: 'PW', label: 'Palau' },
  { code: 'PY', label: 'Paraguay' },
  { code: 'QA', label: 'Qatar' },
  { code: 'RE', label: 'Reunion' },
  { code: 'RO', label: 'Romania' },
  { code: 'RS', label: 'Serbia' },
  { code: 'RU', label: 'Russian Federation' },
  { code: 'RW', label: 'Rwanda' },
  { code: 'SA', label: 'Saudi Arabia' },
  { code: 'SB', label: 'Solomon Islands' },
  { code: 'SC', label: 'Seychelles' },
  { code: 'SD', label: 'Sudan' },
  { code: 'SE', label: 'Sweden' },
  { code: 'SG', label: 'Singapore' },
  { code: 'SH', label: 'Saint Helena' },
  { code: 'SI', label: 'Slovenia' },
  { code: 'SJ', label: 'Svalbard and Jan Mayen' },
  { code: 'SK', label: 'Slovakia' },
  { code: 'SL', label: 'Sierra Leone' },
  { code: 'SM', label: 'San Marino' },
  { code: 'SN', label: 'Senegal' },
  { code: 'SO', label: 'Somalia' },
  { code: 'SR', label: 'Suriname' },
  { code: 'SS', label: 'South Sudan' },
  { code: 'ST', label: 'Sao Tome and Principe' },
  { code: 'SV', label: 'El Salvador' },
  { code: 'SX', label: 'Sint Maarten (Dutch part)' },
  { code: 'SY', label: 'Syrian Arab Republic' },
  { code: 'SZ', label: 'Swaziland' },
  { code: 'TC', label: 'Turks and Caicos Islands' },
  { code: 'TD', label: 'Chad' },
  { code: 'TF', label: 'French Southern Territories' },
  { code: 'TG', label: 'Togo' },
  { code: 'TH', label: 'Thailand' },
  { code: 'TJ', label: 'Tajikistan' },
  { code: 'TK', label: 'Tokelau' },
  { code: 'TL', label: 'Timor-Leste' },
  { code: 'TM', label: 'Turkmenistan' },
  { code: 'TN', label: 'Tunisia' },
  { code: 'TO', label: 'Tonga' },
  { code: 'TR', label: 'Turkey' },
  { code: 'TT', label: 'Trinidad and Tobago' },
  { code: 'TV', label: 'Tuvalu' },
  { code: 'TW', label: 'Taiwan' },
  { code: 'TZ', label: 'United Republic of Tanzania' },
  { code: 'UA', label: 'Ukraine' },
  { code: 'UG', label: 'Uganda' },
  { code: 'UY', label: 'Uruguay' },
  { code: 'UZ', label: 'Uzbekistan' },
  { code: 'VA', label: 'Holy See (Vatican City State)' },
  { code: 'VC', label: 'Saint Vincent and the Grenadines' },
  { code: 'VE', label: 'Venezuela' },
  { code: 'VG', label: 'British Virgin Islands' },
  { code: 'VI', label: 'US Virgin Islands' },
  { code: 'VN', label: 'Vietnam' },
  { code: 'VU', label: 'Vanuatu' },
  { code: 'WF', label: 'Wallis and Futuna' },
  { code: 'WS', label: 'Samoa' },
  { code: 'XK', label: 'Kosovo' },
  { code: 'YE', label: 'Yemen' },
  { code: 'YT', label: 'Mayotte' },
  { code: 'ZA', label: 'South Africa' },
  { code: 'ZM', label: 'Zambia' },
  { code: 'ZW', label: 'Zimbabwe' },
];

export const unis: Record<string, string>[] = [
  { uni: 'University of California-Riverside' },
  { uni: 'University of California-Berkeley' },
  { uni: 'University of California-Davis' },
  { uni: 'University of California-Irvine' },
  { uni: 'University of California-Los Angeles' },
  { uni: 'University of California-San Diego' },
  { uni: 'University of California-San Francisco' },
  { uni: 'University of California-Santa Barbara' },
  { uni: 'University of California-Santa Cruz' },
  { uni: 'Bethesda University of California' },
  { uni: 'California Institute of Integral Studies' },
  { uni: 'California Baptist University' },
  { uni: 'California College of the Arts' },
  { uni: 'University of California-Hastings College of Law' },
  { uni: 'California Institute of Technology' },
  { uni: 'California Lutheran University' },
  { uni: 'California Polytechnic State University-San Luis Obispo' },
  { uni: 'California State University-Bakersfield' },
  { uni: 'California State University-Stanislaus' },
  { uni: 'California State University-Chancellors Office' },
  { uni: 'California State University-San Bernardino' },
  { uni: 'California State Polytechnic University-Pomona' },
  { uni: 'California State University-Chico' },
  { uni: 'California State University-Dominguez Hills' },
  { uni: 'California State University-Fresno' },
  { uni: 'California State University-Fullerton' },
  { uni: 'California State University-East Bay' },
  { uni: 'California State University-Long Beach' },
  { uni: 'California State University-Los Angeles' },
  { uni: 'California State University-Northridge' },
  { uni: 'California State University-Sacramento' },
  { uni: 'California Beauty School' },
  { uni: 'California Christian College' },
  { uni: 'California College San Diego' },
  { uni: 'California Hair Design Academy' },
  { uni: 'California Institute of the Arts' },
  { uni: 'California Maritime Academy' },
  { uni: 'California Western School of Law' },
  { uni: 'DeVry University-California' },
  { uni: 'Dominican University of California' },
  { uni: 'Hair California Beauty Academy' },
  { uni: 'Fuller Theological Seminary in California' },
  { uni: 'The Art Institute of California-Argosy University San Diego' },
  { uni: 'Southern California Seminary' },
  { uni: 'Southern California University of Health Sciences' },
  { uni: 'The Art Institute of California-Argosy University San Francisco' },
  { uni: 'Marymount California University' },
  { uni: "Saint Mary's College of California" },
  { uni: 'Vanguard University of Southern California' },
  { uni: 'Southern California Institute of Architecture' },
  { uni: 'University of Southern California' },
  { uni: 'University of California-System Administration Central Office' },
  { uni: 'Carrington College California-Sacramento' },
  { uni: 'Westminster Theological Seminary in California' },
  { uni: 'California University of Pennsylvania' },
  { uni: 'Carrington College California-San Leandro' },
  { uni: 'California Career School' },
  { uni: 'California State University-San Marcos' },
  { uni: 'Southern California Institute of Technology' },
  { uni: 'University of Phoenix-Southern California Campus' },
  { uni: 'California State University-Monterey Bay' },
  { uni: 'The Art Institute of California-Argosy University Hollywood' },
  { uni: 'Central California School' },
  { uni: "DeVry University's Keller Graduate School of Management-California" },
  { uni: 'The Art Institute of California-Argosy University Los Angeles' },
  { uni: 'Carrington College California-Antioch' },
  { uni: 'Carrington College California-San Jose' },
  { uni: 'Carrington College California-Pleasant Hill' },
  { uni: 'Universal Technical Institute of California Inc' },
  { uni: 'California State University-Channel Islands' },
  { uni: 'The Art Institute of California-Argosy University Orange County' },
  { uni: 'California Healing Arts College' },
  { uni: 'NTMA Training Centers of Southern California' },
  { uni: 'California College of Vocational Careers' },
  { uni: 'University of California-Merced' },
  { uni: 'Carrington College California-Emeryville' },
  { uni: 'California Career College' },
  { uni: 'Universal Technical Institute of Northern California Inc' },
  { uni: 'The Art Institute of California-Argosy University Inland Empire' },
  { uni: 'Career College of California' },
  { uni: 'The Art Institute of California-Argosy University Sacramento' },
  { uni: 'Carrington College California-Stockton' },
  { uni: 'Carrington College California-Citrus Heights' },
  { uni: 'The Art Institute of California-Argosy University-Silicon Valley' },
  { uni: 'California InterContinental University' },
  { uni: 'California Nurses Educational Institute' },
  { uni: 'Academy for Jewish Religion-California' },
  { uni: 'Northern California Institute of Cosmetology Inc' },
  { uni: 'Southern California University SOMA' },
  { uni: 'Touro University California' },
  { uni: 'California University of Management and Sciences' },
  { uni: 'Southern California Health Institute' },
  { uni: 'Carrington College California-Pomona' },
  { uni: 'International Culinary Center-California' },
  { uni: 'California College San Diego' },
  { uni: 'California College San Diego' },
  { uni: 'California Miramar University' },
  { uni: 'California Career Institute' },
  { uni: 'DeVry University-California' },
];

export const majors: Record<string, string>[] = [
  {
    major: 'Computer Science',
  },
  {
    major: 'Computer And Information Systems',
  },

  {
    major: 'Computer Programming And Data Processing',
  },

  {
    major: 'Computer Administration Management And Security',
  },

  {
    major: 'Computer Networking And Telecommunications',
  },

  {
    major: 'Mathematics And Computer Science',
  },

  {
    major: 'Science And Computer Teacher Education',
  },

  {
    major: 'Computer Engineering',
  },

  {
    major: 'General Agriculture',
  },
  {
    major: 'Agriculture Production And Management',
  },
  {
    major: 'Agricultural Economics',
  },
  {
    major: 'Animal Sciences',
  },
  {
    major: 'Food Science',
  },
  {
    major: 'Plant Science And Agronomy',
  },
  {
    major: 'Soil Science',
  },
  {
    major: 'Miscellaneous Agriculture',
  },
  {
    major: 'Forestry',
  },
  {
    major: 'Natural Resources Management',
  },
  {
    major: 'Fine Arts',
  },
  {
    major: 'Drama And Theater Arts',
  },
  {
    major: 'Music',
  },
  {
    major: 'Visual And Performing Arts',
  },
  {
    major: 'Commercial Art And Graphic Design',
  },
  {
    major: 'Film Video And Photographic Arts',
  },
  {
    major: 'Studio Arts',
  },
  {
    major: 'Miscellaneous Fine Arts',
  },
  {
    major: 'Environmental Science',
  },
  {
    major: 'Biology',
  },
  {
    major: 'Biochemical Sciences',
  },
  {
    major: 'Botany',
  },
  {
    major: 'Molecular Biology',
  },
  {
    major: 'Ecology',
  },
  {
    major: 'Genetics',
  },
  {
    major: 'Microbiology',
  },
  {
    major: 'Pharmacology',
  },
  {
    major: 'Physiology',
  },
  {
    major: 'Zoology',
  },
  {
    major: 'Neuroscience',
  },
  {
    major: 'Miscellaneous Biology',
  },
  {
    major: 'Cognitive Science And Biopsychology',
  },
  {
    major: 'General Business',
  },
  {
    major: 'Accounting',
  },
  {
    major: 'Actuarial Science',
  },
  {
    major: 'Business Management And Administration',
  },
  {
    major: 'Operations Logistics And E-Commerce',
  },
  {
    major: 'Business Economics',
  },
  {
    major: 'Marketing And Marketing Research',
  },
  {
    major: 'Finance',
  },
  {
    major: 'Human Resources And Personnel Management',
  },
  {
    major: 'International Business',
  },
  {
    major: 'Hospitality Management',
  },
  {
    major: 'Management Information Systems And Statistics',
  },
  {
    major: 'Miscellaneous Business & Medical Administration',
  },
  {
    major: 'Communications',
  },
  {
    major: 'Journalism',
  },
  {
    major: 'Mass Media',
  },
  {
    major: 'Advertising And Public Relations',
  },
  {
    major: 'Communication Technologies',
  },
  {
    major: 'Information Sciences',
  },
  {
    major: 'Mathematics',
  },
  {
    major: 'Applied Mathematics',
  },
  {
    major: 'Statistics And Decision Science',
  },
  {
    major: 'General Education',
  },
  {
    major: 'Educational Administration And Supervision',
  },
  {
    major: 'School Student Counseling',
  },
  {
    major: 'Elementary Education',
  },
  {
    major: 'Mathematics Teacher Education',
  },
  {
    major: 'Physical And Health Education Teaching',
  },
  {
    major: 'Early Childhood Education',
  },
  {
    major: 'Secondary Teacher Education',
  },
  {
    major: 'Special Needs Education',
  },
  {
    major: 'Social Science Or History Teacher Education',
  },
  {
    major: 'Teacher Education: Multiple Levels',
  },
  {
    major: 'Language And Drama Education',
  },
  {
    major: 'Art And Music Education',
  },
  {
    major: 'Miscellaneous Education',
  },
  {
    major: 'Library Science',
  },
  {
    major: 'Architecture',
  },
  {
    major: 'General Engineering',
  },
  {
    major: 'Aerospace Engineering',
  },
  {
    major: 'Biological Engineering',
  },
  {
    major: 'Architectural Engineering',
  },
  {
    major: 'Biomedical Engineering',
  },
  {
    major: 'Chemical Engineering',
  },
  {
    major: 'Civil Engineering',
  },
  {
    major: 'Electrical Engineering',
  },
  {
    major: 'Engineering Mechanics Physics And Science',
  },
  {
    major: 'Environmental Engineering',
  },
  {
    major: 'Geological And Geophysical Engineering',
  },
  {
    major: 'Industrial And Manufacturing Engineering',
  },
  {
    major: 'Materials Engineering And Materials Science',
  },
  {
    major: 'Mechanical Engineering',
  },
  {
    major: 'Metallurgical Engineering',
  },
  {
    major: 'Mining And Mineral Engineering',
  },
  {
    major: 'Naval Architecture And Marine Engineering',
  },
  {
    major: 'Nuclear Engineering',
  },
  {
    major: 'Petroleum Engineering',
  },
  {
    major: 'Miscellaneous Engineering',
  },
  {
    major: 'Engineering Technologies',
  },
  {
    major: 'Engineering And Industrial Management',
  },
  {
    major: 'Electrical Engineering Technology',
  },
  {
    major: 'Industrial Production Technologies',
  },
  {
    major: 'Mechanical Engineering Related Technologies',
  },
  {
    major: 'Miscellaneous Engineering Technologies',
  },
  {
    major: 'Materials Science',
  },
  {
    major: 'Nutrition Sciences',
  },
  {
    major: 'General Medical And Health Services',
  },
  {
    major: 'Communication Disorders Sciences And Services',
  },
  {
    major: 'Health And Medical Administrative Services',
  },
  {
    major: 'Medical Assisting Services',
  },
  {
    major: 'Medical Technologies Technicians',
  },
  {
    major: 'Health And Medical Preparatory Programs',
  },
  {
    major: 'Nursing',
  },
  {
    major: 'Pharmacy Pharmaceutical Sciences And Administration',
  },
  {
    major: 'Treatment Therapy Professions',
  },
  {
    major: 'Community And Public Health',
  },
  {
    major: 'Miscellaneous Health Medical Professions',
  },
  {
    major: 'Area Ethnic And Civilization Studies',
  },
  {
    major: 'Linguistics And Comparative Language And Literature',
  },
  {
    major: 'French German Latin And Other Common Foreign Language Studies',
  },
  {
    major: 'Other Foreign Languages',
  },
  {
    major: 'English Language And Literature',
  },
  {
    major: 'Composition And Rhetoric',
  },
  {
    major: 'Liberal Arts',
  },
  {
    major: 'Humanities',
  },
  {
    major: 'Intercultural And International Studies',
  },
  {
    major: 'Philosophy And Religious Studies',
  },
  {
    major: 'Theology And Religious Vocations',
  },
  {
    major: 'Anthropology And Archeology',
  },
  {
    major: 'Art History And Criticism',
  },
  {
    major: 'History',
  },
  {
    major: 'United States History',
  },
  {
    major: 'Cosmetology Services And Culinary Arts',
  },
  {
    major: 'Family And Consumer Sciences',
  },
  {
    major: 'Military Technologies',
  },
  {
    major: 'Physical Fitness Parks Recreation And Leisure',
  },
  {
    major: 'Construction Services',
  },
  {
    major: 'Electrical, Mechanical, And Precision Technologies And Production',
  },
  {
    major: 'Transportation Sciences And Technologies',
  },
  {
    major: 'Multi/Interdisciplinary Studies',
  },
  {
    major: 'Court Reporting',
  },
  {
    major: 'Pre-Law And Legal Studies',
  },
  {
    major: 'Criminal Justice And Fire Protection',
  },
  {
    major: 'Public Administration',
  },
  {
    major: 'Public Policy',
  },
  {
    major: "N/A (less than bachelor's degree)",
  },
  {
    major: 'Physical Sciences',
  },
  {
    major: 'Astronomy And Astrophysics',
  },
  {
    major: 'Atmospheric Sciences And Meteorology',
  },
  {
    major: 'Chemistry',
  },
  {
    major: 'Geology And Earth Science',
  },
  {
    major: 'Geosciences',
  },
  {
    major: 'Oceanography',
  },
  {
    major: 'Physics',
  },
  {
    major: 'Multi-Disciplinary Or General Science',
  },
  {
    major: 'Nuclear, Industrial Radiology, And Biological Technologies',
  },
  {
    major: 'Psychology',
  },
  {
    major: 'Educational Psychology',
  },
  {
    major: 'Clinical Psychology',
  },
  {
    major: 'Counseling Psychology',
  },
  {
    major: 'Industrial And Organizational Psychology',
  },
  {
    major: 'Social Psychology',
  },
  {
    major: 'Miscellaneous Psychology',
  },
  {
    major: 'Human Services And Community Organization',
  },
  {
    major: 'Social Work',
  },
  {
    major: 'Interdisciplinary Social Sciences',
  },
  {
    major: 'General Social Sciences',
  },
  {
    major: 'Economics',
  },
  {
    major: 'Criminology',
  },
  {
    major: 'Geography',
  },
  {
    major: 'International Relations',
  },
  {
    major: 'Political Science And Government',
  },
  {
    major: 'Sociology',
  },
  {
    major: 'Miscellaneous Social Sciences',
  },
];
